...
void stack_check_init(void)
{
    ...
    // volatile prevents compiler from replacing this loop with a tail-called memset,
    // which would push its frame into the fill range and corrupt its own return address
    while (ptr < end) {
        *(volatile uint8_t*)ptr = 0xAA;
        ptr++;
    }
}
